...
func TestWithReplica(t *testing.T) {
	...
	if err = os.RemoveAll(wd + "/replica"); err != nil {
		slog.Warn(err.Error())
	}
	if err = os.RemoveAll(wd + "/leader"); err != nil {
		slog.Warn(err.Error())
	}
	...
	checkInsert(t, mux_leader, feature)

	checkSelect(t, mux_leader, make([]float64, 0), []geojson.Feature{*feature})
	checkSelect(t, mux_replica, make([]float64, 0), []geojson.Feature{*feature})

	featureNew := geojson.NewFeature(orb.Point{rand.Float64(), rand.Float64()})
	featureNew.ID = "t"

	checkReplace(t, mux_leader, featureNew)
	checkSelect(t, mux_leader, make([]float64, 0), []geojson.Feature{*featureNew})
	checkSelect(t, mux_replica, make([]float64, 0), []geojson.Feature{*featureNew})

	checkDelete(t, mux_leader, "t")
	checkSelect(t, mux_leader, make([]float64, 0), []geojson.Feature{})
	checkSelect(t, mux_replica, make([]float64, 0), []geojson.Feature{})
...
func TestModificationOnReplica(t *testing.T) {
	wd, err := os.Getwd()
	if err != nil {
		t.Fatal(err)
	}
	...
	checkError(t, mux_replica, "POST", "/insert", bytes.NewReader(b))
	checkError(t, mux_replica, "POST", "/replace", bytes.NewReader(b))
	...
	checkError(t, mux_replica, "POST", "/delete", bytes.NewReader(b))
